...
* Custom link options
    ...
    <%= paginate @users, :link_options => {:data => {:push => true}} %>
  This would add <tt>data-push="true"</tt> to all the links inside.

* Ajax links (crazy simple, but works perfectly!)

    <%= paginate @users, :remote => true %>
  This would add <tt>data-remote="true"</tt> to all the links inside. It's a shortcut for <tt>:link_options => {:remote => true}<tt>.
